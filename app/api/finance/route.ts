 ...
 *
 * ingresos anteriores - gastos anteriores
 */
export async function GET(request: NextRequest) {
  ...
  /*
   * SALDO DE TODOS LOS MESES ANTERIORES
   */
  const [previousSummary] = await db
    ...
    .where(
      and(
        ...
        lt(transactions.transactionDate, monthInfo.start),
      ),
    );

  const previousBalance =
    Number(previousSummary?.income ?? 0) -
    Number(previousSummary?.expense ?? 0);
  ...
  const transactionRows = await db
    ...
    .where(
      and(
        eq(transactions.userId, user.id),
        gte(transactions.transactionDate, monthInfo.start),
        lt(transactions.transactionDate, monthInfo.end),
      ),
    )
    .orderBy(
  ...
  const [budgetRow] = await db
    ...
    .where(
      and(
        ...
        eq(monthlyBudgets.year, monthInfo.year),
        eq(monthlyBudgets.month, monthInfo.month),
      ),
    )
    .limit(1);
...
export async function PATCH(request: NextRequest) {
  ...
  if (body.kind === "goal") {
    ...
    const contribution = Number(body.amount);
  ...
  if (existing) {
    await db
      ...
      .where(
        and(
          eq(monthlyBudgets.id, existing.id),
          eq(monthlyBudgets.userId, user.id),
        ),
      );
  } else {
